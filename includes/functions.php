...
//
function fileUploader($file, $file_type, $keep_file_name = 'no', $dir = 'uploads')
{
    if($keep_file_name == 'no'){
        ...
        if (move_uploaded_file($file["tmp_name"], $target_file . '.' . $imageFileType)) {
            ...
            if (is_writable($target_dir)) {
                if (! is_writeable($target_file . '.' . $imageFileType)) {
                    echo $target_file . '.' . $imageFileType . $file["name"];
                   
                    die("Cannot write to destination file");
                }
            } else {
                die("Cannot write to destination folder");
            }
